...
app.delete("/api/folders", async (req, res) => {
  try {
    ...
    res.json({ message: "All folders deleted" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.delete("/api/recipe_connections/:id", async (req, res) => {
  try {
    await pool.query("DELETE FROM recipe_connections WHERE id = $1", [req.params.id]);
    res.json({ message: "Recipe connection deleted" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete("/api/recipe_connections", async (req, res) => {
  try {
    await pool.query("DELETE FROM recipe_connections");
    res.json({ message: "All recipe connections deleted" });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete("/api/audit_logs/:id", async (req, res) => {
  try {
